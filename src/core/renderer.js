...
function pixelRatioFor(viewportWidth) {
  const cap = viewportWidth > 1600 ? Math.min(ratioCap, 1.25) : ratioCap;
  return Math.min(window.devicePixelRatio || 1, cap);
}

export function createRenderer(canvas, scene, camera, quality = null) {
  if (quality) ratioCap = quality.pixelRatio;
  const renderer = new THREE.WebGLRenderer({
    canvas,
    antialias: true,
    powerPreference: 'high-performance',
    stencil: false,
    // The poster is built by reading this canvas back. Without this the buffer
    // is cleared on present and the capture comes out blank on most drivers.
  ...
  const bloom = new UnrealBloomPass(
    ...
    0.3, // strength, driven by the sunrise
    0.8, // radius
    // Threshold high on purpose. A morning sky sits at around 0.9 after tone
    // mapping, so a lower cut blooms the entire horizon into a white band and
    // the rooflines dissolve into it.
    0.94
  );
